...
class ModelTests(TestCase):
    ...
    def test_new_user_without_username_pass(self):
        """Test that creating a user will pass"""
        user = get_user_model().objects.create_user(
                userEmailAddress = "test1@example.com",
                password = "testpass123",
            )

    def test_create_users_with_unique_autogenerated_usernames(self):
        """Test creating multiple users without a username generates unique usernames"""
        email1 = "test1@example.com"
        email2 = "test2@example.com"
        ...
        self.assertIsNotNone(user1.userUsername)
        self.assertIsNotNone(user2.userUsername)
        self.assertNotEqual(user1.userUsername, user2.userUsername)
        self.assertTrue(get_user_model().objects.filter(userUsername=user1.userUsername).exists())
        self.assertTrue(get_user_model().objects.filter(userUsername=user2.userUsername).exists())
